...
void setup() {
  ...
  if(DEBUG_MODE==true) {
      Serial.println((String)"modelNumber: "+modelNumber);
      delay(5);
      Serial.println((String)"cursorSpeedCounter: "+cursorSpeedCounter);
      delay(5);
      Serial.println((String)"cursorDelay: "+cursorParams[cursorSpeedCounter]._delay);
      delay(5);
      Serial.println((String)"cursorFactor: "+cursorParams[cursorSpeedCounter]._factor);
      delay(5);
      Serial.println((String)"cursorMaxSpeed: "+cursorParams[cursorSpeedCounter]._maxSpeed);
      delay(5);
...
void bluetoothConfigStatus(void) {
  EEPROM.get(0, modelNumber);                         //Assign value of EEPROM memory at index zero (0) to int variable BT_EEPROM
  delay(10);
  bluetoothConfigDone = (modelNumber == 3) ? 1 : 0;  //Define the bluetoothConfigDone to 0 if the device is set for the first time
  delay(10);
}
...
void bluetoothConfigSequence(void) {
  ...
  EEPROM.put(0, modelNumber);                              //Save the configuration completed value at EEPROM address location 0
  delay(15);
  EEPROM.put(2, cursorSpeedCounter);                              //Save the default cursor speed counter value at EEPROM address location 2
  delay(15);
}
